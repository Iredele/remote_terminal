...
static void bridge_tick (bridge_t *bridge)
{
   ...
   switch (bridge->state)
   {
      case BRIDGE_STATE_INIT:
         ...
         ESP_LOGI (TAG, "bridge starting (%s <-> %s)", PORT_NAME (port_a),
                   PORT_NAME (port_b));
         break;
      ...
      case BRIDGE_STATE_WAIT:
      {
         ...
         {
            if (!a_conn && !b_conn)
               ESP_LOGW (TAG, "%s and %s disconnected", PORT_NAME (port_a),
                         PORT_NAME (port_b));
            else
               ESP_LOGW (TAG, "%s disconnected",
                         !a_conn ? PORT_NAME (port_a) : PORT_NAME (port_b));
      ...
      case BRIDGE_STATE_CONNECT:
      {
         ...
         if (!a_ok || !b_ok)
         {
            if (bridge->_idle_ticks == 0 || bridge->_idle_ticks % 500 == 0)
               ESP_LOGW (TAG, "%s connect failed, retrying",
                         !a_ok ? PORT_NAME (port_a) : PORT_NAME (port_b));
            bridge->_idle_ticks++;
            bridge->state = BRIDGE_STATE_CONNECT;
            break;
         }

         ESP_LOGI (TAG, "bridge open (%s <-> %s)", PORT_NAME (port_a),
                   PORT_NAME (port_b));
         bridge->is_open     = true;
         bridge->_idle_ticks = 0;
         bridge->state       = BRIDGE_STATE_WAIT;
      ...
      case BRIDGE_STATE_SEND:
         ...
         if (a_bytes > 0)
         {
            ...
            ESP_LOGD (TAG, "%s -> %s: %u bytes", PORT_NAME (port_a),
                      PORT_NAME (port_b), rlen);
         }
         ...
         if (b_bytes > 0)
         {
            ...
            ESP_LOGD (TAG, "%s -> %s: %u bytes", PORT_NAME (port_b),
                      PORT_NAME (port_a), rlen);
         }
         bridge->_idle_ticks = 0;
         bridge->state       = BRIDGE_STATE_WAIT;
         break;
