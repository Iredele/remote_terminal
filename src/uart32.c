...
uart32_err_t UARTInit (uint32_t port, const uart32_cfg_t *config)
{
   ...
   // set UART pins(TX: IO4, RX: IO5, RTS: IO18, CTS: IO19, DTR: UNUSED, DSR:
   // UNUSED)
   err = uart_set_pin (port, config->tx_pin, config->rx_pin, config->rts_pin,
                       config->cts_pin);
   if (err != ESP_OK)
   {
      return (UART_ERR_INIT);
   ...
   err = uart_driver_install (port, config->buffer_size, config->buffer_size,
                              10, &uart_queue[port], 0);
   if (err != ESP_OK)
   {
      return (UART_ERR_INIT);
   }
...
uint32_t UARTWrite (uint32_t port, const uint8_t *data, uint32_t len)
{
   ...
   {
      ESP_LOGE ("uart32",
                "uart_write_bytes failed on port %lu, requested %lu bytes",
...
uint32_t UARTRead (uint32_t port, uint8_t *buff, uint32_t len)
{
   ...
   if (rlen < 0)
   {
      ESP_LOGW ("uart32", "uart_read_bytes failed on port %lu",
                (unsigned long)port);
      return 0;
   }
