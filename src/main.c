...
static const uart32_cfg_t uart_cfg = {
    115200, UART_DATA_8_BITS, UART_PARITY_DISABLE, UART_STOP_BITS_1, UART_HW_FLOWCTRL_DISABLE, UART2_TX_PIN, UART2_RX_PIN, UNUSED_PIN, UNUSED_PIN, UART_BUFFER_SZ
};
...
void app_main(void)
{
    ...
    {
        // if the NVS partition is corrupted or needs to be erased, erase it and retry initialization
    ...
    while (WIFIIsConnected() != true) // wait for wifi to connect
    {
        ...
        retries++;
        ESP_LOGI(TAG, "Waiting for WiFi connection (attempt %d/%d)...", retries, WIFI_RETRY_MAX);
