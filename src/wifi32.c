...
wifi_err_t WIFIInit(void)
{
    ...
    err = esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &ip_event_handler, NULL, &instance_got_ip);
    if (err != ESP_OK)
    {
        return (WIFI_ERR_INIT);
    }
...
wifi_err_t WIFIConnect(const char *ssid, const char *password)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGW(TAG, "Failed to disable WiFi power saving: %s", esp_err_to_name(err));
    }
    else
    {
    ...
    const size_t password_len = min(strlen(password), sizeof(wifi_config.sta.password));
...
{
    return sta_connected && ip_obtained;
}
